---
locale: de
slug: terapias-el-templo
title: 'Terapias El Templo'
summary: 'Ein Zentrum für ganzheitliche Therapien auf Teneriffa. Wir haben den Weg von der ersten Anzeige bis zur gebuchten Sitzung gebaut: Anzeigen, Landingpage, Buchung und Nachfassen, verbunden.'
order: 10
client: 'Terapias El Templo'
sector: 'Zentrum für ganzheitliche Therapien'
location: 'Tenerife'
results:
  - label: 'Kosten pro wahrgenommenem Termin'
    value: '[PENDIENTE]'
  - label: 'Zurückgewonnene Ausfälle'
    value: '[PENDIENTE]'
  - label: 'Kunden, die wiederkommen'
    value: '[PENDIENTE]'
servicesUsed: []
---

Die vollständige Fallstudie entsteht in `feature/casos`.
